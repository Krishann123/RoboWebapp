...
const $$Navbar = createComponent(async ($$result, $$props, $$slots) => {
  ...
  )} </ul> <!-- CTA Button --> <div class="flex items-center gap-4 mr-5"> <div class="flex sm:gap-4"> <a${addAttribute(getPath(buttonLink), "href")} target="_blank" class="bg-white text-blue-900 text-center text-xs lg:text-lg font-medium rounded-full px-4 py-2 hover:bg-indigo-400 hover:text-white shadow-2xl transition"> ${buttonText} </a> </div> </div> <!-- Hamburger Icon (shown on small screens) --> <button class="md:hidden mr-5 text-white hover:scale-120" id="menu-toggle"> <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path> </svg> </button> </div> <!-- Dropdown Menu (hidden by default, shown when menu is toggled) --> <div id="mobile-menu" class="fixed top-14 right-0 p-3 z-50 md:hidden hidden mt-5 text-center shadow-lg bg-[#2F1573]/60 backdrop-blur-md"> <ul class="space-y-2 text-white font-medium"> <!-- Dynamic page links --> ${links.map((item) => renderTemplate`<li> <a${addAttribute(getPath(item.path), "href")} class="block hover:text-[#C084FC] hover:scale-120"> ${item.name} </a> </li>`)} <!-- Back to Main Site Link (Mobile) --> <li> <a href="/home" class="block text-yellow-300 hover:text-yellow-100 hover:scale-120 font-bold">
← Back to Main Site
